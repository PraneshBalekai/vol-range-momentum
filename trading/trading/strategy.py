...
def load_noise_area(df: pd.DataFrame, lookback_days: int, volatility_multiplier: float) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    """
    assert len(df) >= lookback_days, f"Not enough input data in the dataframe, lookback days: {lookback_days}, length of df: {len(df)}"
    ...
    df['vwap'] = df.groupby(df.index.date)['typical_px'].cumsum() / df.groupby(df.index.date)['volume'].cumsum()

    # store daily close and open values
    daily_data = df.groupby('date').agg(day_open=('open','first'), day_close=('close', 'last'))
    daily_data['prev_close'] = daily_data['day_close'].shift(1)
   
    # stats for vol scaling
    daily_data['mu'] = daily_data['day_close'].pct_change().rolling(lookback_days).mean().shift(1) # see pg.14 on the paper
    daily_data['sigma'] = np.nan

    for window in daily_data.rolling(lookback_days+2): # last row is 't', the window's first row pct_change == NaN, so we add +2.
        if len(window) < (lookback_days+2):
            continue
        sigma = (window[1:lookback_days]['day_close'].pct_change() - window.loc[window.index[-1], 'mu'])
        sigma = sigma**2
    ...
    avg_move = pd.pivot_table(df, 'move', index='date', columns='minute').ffill() #ffill to fill up data for those days where market closes at 13:00
    avg_move = avg_move.rolling(lookback_days, min_periods=lookback_days).mean()
    latest_avg = avg_move[-lookback_days:].mean()

    avg_move = pd.melt(avg_move.reset_index(), id_vars='date', value_vars=list(avg_move.columns), var_name='minute', value_name='avg_move')
    df = df.merge(avg_move, left_on=['date', 'minute'], right_on=['date', 'minute'])

    df['datetime'] = pd.to_datetime(df['date'].astype(str) + df['minute'].astype(str), format='%Y-%m-%d%H:%M:%S')

    df.index = df['datetime']
    df = df[['date', 'minute', 'open', 'close', 'high', 'low', 'volume', 'vwap', 'day_open', 'day_close', 'prev_close', 'move', 'avg_move', 'mu', 'sigma']]

    df['upper_bound'] = df.apply(lambda x: max(x['prev_close'], x['day_open']) * (1 + (volatility_multiplier*x['avg_move'])), axis=1)
    df['lower_bound'] = df.apply(lambda x: min(x['prev_close'], x['day_open']) * (1 - (volatility_multiplier*x['avg_move'])), axis=1)
